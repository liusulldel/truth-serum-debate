...
*(a) Production engineering as applied mechanism design.* I have built and maintained three LangGraph pipelines in Python. The most relevant here is **ADM Corp** — a 137-agent four-phase research-paper refinement orchestration (40 research → 35 writing → 30 red-team → 23 QA agents) with a Chief-Editor rewrite gate, veto logic, and budget-aware scheduling — where the institution-design problem is exactly the kind Maxim and Peter's paper studies in miniature: how does a multi-agent system allocate sub-tasks so that local agent behavior aggregates to a desired global property (here, paper quality; in their data, occupational output)? The day-to-day work is: specify agent roles, design payoff structures so the aggregation behaves, and instrument it. That is mechanism design with a `pytest` suite. The framing is written up in `docs/CORPORATE_MGMT_ANGLE.md` of the repo linked below: LangGraph as institutional design with a compiler.
